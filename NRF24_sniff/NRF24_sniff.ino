...
#define RF_MAX_ADDR_WIDTH              (3)                                                      // Maximum address width, in bytes. MySensors use 5 bytes for addressing, where lowest byte is for node addressing.
...
#define DEFAULT_RF_CHANNEL             (0x39)                                                     // 76 = Default channel for MySensors.
...
#define DEFAULT_RADIO_ID               ((uint64_t)0x0000000055LL)                               // 0xA8A8E1FC00LL = MySensors v2 (1.4) default
#define DEFAULT_RF_CRC_LENGTH          (0)                                                      // Length (in bytes) of NRF24 CRC
...
static void handleNrfIrq()
{
  ...
  while (radio.available())
  {
    ...
    if (!packetBuffer.full())
    {
      ...
      if (GET_PAYLOAD_LEN(p) == 0 ) //<= MAX_RF_PAYLOAD_SIZE)
      {
